...
- Canonical production target: Vercel (auto-deploy via Git integration).
...
Commit, merge, and deploy completed, verified work automatically unless the request explicitly says not to deploy. Production deploys to Vercel are triggered by pushing to the repo's default branch; do not add or restore GitHub Pages deployment.
...
- Verify deployment-related edits against the README before changing docs.
